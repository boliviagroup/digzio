  ...
  try {
    ...
    if (instId) {
      if (searchVal) {
        ...
        query = `SELECT u.user_id, u.first_name, u.last_name, u.email, sp.student_number, sp.id_number, sp.nsfas_status, sp.year_of_study, sp.qualification, sp.campus, sp.gender, sp.type_of_funding, CASE WHEN l.is_active THEN 'ACTIVE' ELSE 'INACTIVE' END AS lease_status, l.start_date, l.end_date, p.title AS property_title, p.city FROM student_profiles sp JOIN users u ON u.user_id = sp.student_id LEFT JOIN leases l ON l.student_id = u.user_id AND l.is_active = true LEFT JOIN properties p ON p.property_id = l.property_id WHERE sp.institution_id = $1 AND (u.first_name ILIKE $4 OR u.last_name ILIKE $4 OR sp.student_number ILIKE $4) ORDER BY u.last_name ASC LIMIT $2 OFFSET $3`;
        ...
        countParams = [instId];
        query = `SELECT u.user_id, u.first_name, u.last_name, u.email, sp.student_number, sp.id_number, sp.nsfas_status, sp.year_of_study, sp.qualification, sp.campus, sp.gender, sp.type_of_funding, CASE WHEN l.is_active THEN 'ACTIVE' ELSE 'INACTIVE' END AS lease_status, l.start_date, l.end_date, p.title AS property_title, p.city FROM student_profiles sp JOIN users u ON u.user_id = sp.student_id LEFT JOIN leases l ON l.student_id = u.user_id AND l.is_active = true LEFT JOIN properties p ON p.property_id = l.property_id WHERE sp.institution_id = $1 ORDER BY u.last_name ASC LIMIT $2 OFFSET $3`;
      ...
      if (searchVal) {
        ...
        query = `SELECT u.user_id, u.first_name, u.last_name, u.email, sp.student_number, sp.id_number, sp.nsfas_status, sp.year_of_study, sp.qualification, sp.campus, sp.gender, sp.type_of_funding, CASE WHEN l.is_active THEN 'ACTIVE' ELSE 'INACTIVE' END AS lease_status, l.start_date, l.end_date, p.title AS property_title, p.city FROM student_profiles sp JOIN users u ON u.user_id = sp.student_id LEFT JOIN leases l ON l.student_id = u.user_id AND l.is_active = true LEFT JOIN properties p ON p.property_id = l.property_id WHERE (u.first_name ILIKE $3 OR u.last_name ILIKE $3 OR sp.student_number ILIKE $3) ORDER BY u.last_name ASC LIMIT $1 OFFSET $2`;
        ...
        params = [limit, offset];
        countParams = [];
        query = `SELECT u.user_id, u.first_name, u.last_name, u.email, sp.student_number, sp.id_number, sp.nsfas_status, sp.year_of_study, sp.qualification, sp.campus, sp.gender, sp.type_of_funding, CASE WHEN l.is_active THEN 'ACTIVE' ELSE 'INACTIVE' END AS lease_status, l.start_date, l.end_date, p.title AS property_title, p.city FROM student_profiles sp JOIN users u ON u.user_id = sp.student_id LEFT JOIN leases l ON l.student_id = u.user_id AND l.is_active = true LEFT JOIN properties p ON p.property_id = l.property_id ORDER BY u.last_name ASC LIMIT $1 OFFSET $2`;
...
router.get('/dashboard/report', authenticate, requireInstitution, async (req, res) => {
  try {
    ...
    const studentsQ = instId
      ? await pool.query(`SELECT u.first_name, u.last_name, u.email, sp.student_number, sp.id_number, sp.nsfas_status, sp.year_of_study, sp.qualification, sp.campus, sp.gender, CASE WHEN l.is_active THEN 'ACTIVE' ELSE 'INACTIVE' END AS lease_status, l.start_date, l.end_date, l.monthly_rent, p.title AS property_title, p.address_line_1 AS address, p.city, p.province, p.posa_code, p.is_nsfas_accredited AS nsfas_accredited FROM student_profiles sp JOIN users u ON u.user_id = sp.student_id LEFT JOIN leases l ON l.student_id = u.user_id AND l.is_active = true LEFT JOIN properties p ON p.property_id = l.property_id WHERE sp.institution_id = $1 ORDER BY u.last_name ASC`, [instId])
      : await pool.query(`SELECT u.first_name, u.last_name, u.email, sp.student_number, sp.id_number, sp.nsfas_status, sp.year_of_study, sp.qualification, sp.campus, sp.gender, CASE WHEN l.is_active THEN 'ACTIVE' ELSE 'INACTIVE' END AS lease_status, l.start_date, l.end_date, l.monthly_rent, p.title AS property_title, p.address_line_1 AS address, p.city, p.province, p.posa_code, p.is_nsfas_accredited AS nsfas_accredited FROM student_profiles sp JOIN users u ON u.user_id = sp.student_id LEFT JOIN leases l ON l.student_id = u.user_id AND l.is_active = true LEFT JOIN properties p ON p.property_id = l.property_id ORDER BY u.last_name ASC`);
