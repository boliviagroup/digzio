...
"""
Seed leases for the 5 Siwedi demo students using the new /posa/seed-leases endpoint.
"""
...
for attempt in range(3):
    try:
        r = requests.post(f"{BASE}/properties/posa/seed-leases",
                          json=lease_payload,
                          headers=PROV_HEADERS, timeout=30)
        print(f"   Seed leases: {r.status_code} {r.text[:300]}")
        break
    except Exception as e:
        print(f"   Attempt {attempt+1} failed: {e}")
        time.sleep(5)

# Verify POSA students endpoint
print("\n4. Verifying POSA students endpoint...")
for attempt in range(3):
    try:
        r = requests.get(
        ...
        if r.status_code == 200:
            ...
            for st in d.get('students', []):
                print(f"     - {st.get('first_name')} {st.get('last_name')} | {st.get('student_number', 'N/A')} | {st.get('type_of_funding', 'N/A')}")
        else:
